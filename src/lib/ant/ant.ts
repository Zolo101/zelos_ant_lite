...
class Ant {
    ...
    moveForward(magnitude: number = 1) {
        switch (this.rotation) {
            case AntRotation.North: return this.moveBy(0, -magnitude)
            case AntRotation.East: return this.moveBy(magnitude, 0)
            case AntRotation.South: return this.moveBy(0, magnitude)
            case AntRotation.West: return this.moveBy(-magnitude, 0)
        }
    }
